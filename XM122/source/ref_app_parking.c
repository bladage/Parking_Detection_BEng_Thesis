...
int acconeer_main(int argc, char *argv[])
{
	...
	acc_integration_set_periodic_wakeup((uint32_t)(DETECTOR_SWEEP_PERIOD_S * 1000));
	...
	if (status)
	{
		acc_bluetooth_beacon_init(DETECTOR_SWEEP_PERIOD_S * 1000);
	}
	...
	while (status)
	{
		...
		if (status)
		{
			...
			uint16_t adv_data[2];
			adv_data[0] = sweep_index;
			adv_data[1] = detection ? 1U : 0U;
