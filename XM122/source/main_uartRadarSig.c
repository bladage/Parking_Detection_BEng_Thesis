...
/** 
  @brief  The function maps every data type to uint8_t and it builts 
          ...
          metadata
**/ 
static void create_uart_frame(uint8_t *byte_buffer, const uint16_t uart_frame_size, uint16_t *signal)
{
  ...
  for(uint16_t idx=2; idx <= uart_frame_size/2; idx++)
  {
    //tempH = 2*idx-2;
    //tempL = 2*idx-1;
...
int main(void)
{
	...
	acc_service_envelope_result_info_t result_info;             // missed_data, sensor_communication_error, data_saturated, data_quality_warning
        ...
        */
        ...
        ////const uint16_t uart_frame_size = 2*metadata.data_length+2;
        //const uint16_t uart_frame_size = 2*SIGNAL_SIZE+2;
        ...
        //uint16_t signal[SIGNAL_SIZE];
        //uint16_t uart_frame_size = 2*metadata.data_length+2;
        //uint8_t byte_buffer[uart_frame_size];
        //uint16_t idx; //, tempH, tempL;

        //// built the uart buffer (byte_buffer) which is a 8bit array       
        //create_uart_frame(byte_buffer, uart_frame_size, envelope_data);
	...
	while(true)
	{
          ...
          if(acc_service_envelope_get_next(handle, envelope_data, metadata.data_length, &result_info))
          {
            for(idx=0; idx < metadata.data_length; idx++)
            {
              // TBD: built a UART frame and send that; keep in mind: UART=uint8!
              printf("%d\n", envelope_data[idx]);
            }
            ...
            for(idx=0; idx < uart_frame_size; idx++)
            {
