    ...
    */
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        if(p_evt->params.rx_data.p_data[0] == MSG_SEND_META_DATA && flag_setup)
        {
          ...
          create_msg_send_metadata(byte_msg, &metadata_radar, offset_xyz);   // build message
        ...
        else if(p_evt->params.rx_data.p_data[0] == MSG_SETUP_CONFIG)
        {
          ...
          // Started app timers here before
          
          // TODO setup sensors (radar, lidar)
...
static void apptimer_send_mmc_handler(void * p_context)
{
  ...
  for(uint8_t k=0; k < NUMBER_MGF_SENSORS; k++)
  {
    ...
    //TODO set just one mgf sensor active due to their same I2C addresses... --> hardware workaround is needed for this...
  }
...
int main(void)
{
  ...
  */ 
  ...
  /*
    Get the data to configure the sensors' parameters
  */
