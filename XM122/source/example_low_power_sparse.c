...
#define SAMPLING_MODE B
#endif

#define SELECTED_SAMPLING_MODE (EVALUATOR(ACC_SERVICE_SPARSE_SAMPLING_MODE_, SAMPLING_MODE))
...
static acc_service_handle_t create_sparse_service(void)
{
	...
	if (success)
	{
		...
		acc_service_hw_accelerated_average_samples_set(sparse_configuration, HWAAS);
		acc_service_sparse_configuration_sweeps_per_frame_set(sparse_configuration, SPF);
		acc_service_sparse_sampling_mode_set(sparse_configuration, SELECTED_SAMPLING_MODE);
		acc_service_sparse_downsampling_factor_set(sparse_configuration, DOWNSAMPLING_FACTOR);
...
int acconeer_main(int argc, char *argv[])
{
	...
	acc_service_handle_t handle = create_sparse_service();
	if (handle == NULL)
	{
	...
	while (true)
	{
		...
		execute_sparse_service(handle, &result[0], &result[1]);
